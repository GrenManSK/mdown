...
async fn resolve_web_download(url: &str, handle_id: String) -> String {
    ...
    if let Some(id) = utils::resolve_regex(&url) {
        ...
        println!("[web @{}] Found {}", handle_id, id);
        ...
        println!("[web @{}] Didn't find any id", handle_id);
    }
    if manga_name.eq("!") {
        String::from("!")
    } else {
...
async fn handle_client(mut stream: std::net::TcpStream) {
    ...
    println!("[web] Connection from: {}", addr);
...
fn parse_request(
    ...
    if parts.len() >= 2 {
        if parts[1].starts_with("/manga?") && parts[1].contains(&url_param) {
            println!("[web] REQUEST RECEIVED");
            println!("[web] REQUEST Type: download");
            ...
            if ARGS.log {
                println!("[web] REQUEST RECEIVED");
                println!("[web] REQUEST Type: progress");
            }
            let query_params: HashMap<_, _> = parts[1]
                .split('?')
                .nth(1)
...
pub(crate) async fn web() {
    ...
    println!("[web] Server listening on 127.0.0.1:8080");
