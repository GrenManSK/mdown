                        ...
                        if let Some(name_data) = obj.get("data") {
                            if let Some(attr_data) = name_data.get("attributes") {
                                if let Some(title_data) = attr_data.get("title") {
                                    if let Some(manga_name) =
                                        title_data.get("en").and_then(Value::as_str)
                                    {
                                        let going_offset = arg_offset;
                                        for _ in 0..2 {
                                            match get_manga(id, going_offset).await {
                                                Ok((json, _offset)) => {
                                                    download_manga(json, language, manga_name)
                                                        .await;
                                                }
                                        ...
                                        string(
                                            ...
                                            0,
                                            format!(
                                                "Ending session: {} has been downloaded",
                                                manga_name
                                            )
                                            .as_str(),
                                        );
                        ...
                        if let Some(array_item) = data_array.get(item) {
                            if let Some(field_value) = array_item.get("id") {
                                ...
                                string(
                                    2,
                                    0,
                                    format!(" ({}) Found chapter with id: {}", item as i32, id)
                                        .as_str(),
                                );
                                if let Some(chapter_attr) = array_item.get("attributes") {
                                    if let Some(lang) = chapter_attr
                                        .get("translatedLanguage")
                                        .and_then(Value::as_str)
                                    {
                                        if let Some(pages) =
                                            chapter_attr.get("pages").and_then(Value::as_u64)
                                        {
                                            if let Some(chapter_num) =
                                                chapter_attr.get("chapter").and_then(Value::as_str)
                                            {
                                                let title;
                                                let vol;
                                                if let Some(title_temp) = chapter_attr
                                                    .get("title")
                                                    .and_then(Value::as_str)
                                                {
                                                    title = title_temp;
                                                ...
                                                if let Some(vol_temp) = chapter_attr
                                                    .get("volume")
                                                    .and_then(Value::as_str)
                                                {
                                                ...
                                                let folder_path = format!(
                                                    "{} - {}Ch.{} - {}",
                                                    manga_name, vol, chapter_num, title
                                                );
                                                ...
                                                .is_ok()
                                                {
                                                    string(3,0,format!("  Skipping because file is already downloaded {}",format!(
                                                        "{}",
                                                        folder_path
                                                            .replace('<', "")
                                                            .replace('>', "")
                                                    ...
                                                    && (chapter_num != "This is test")
                                                {
                                                    ...
                                                    string(3,0,format!("  Metadata: Language: {};Pages: {};Vol: {};Chapter: {};Title: {}", lang, pages, vol, chapter_num, title).as_str());
                                                    ...
                                                    .await;
                                                    let _ = tokio::fs::remove_dir_all(folder_path)
                                                        .await;
...
async fn download_chapter(
    ...
    string(
        5,
        0,
        format!("  Downloading images in folder: {}/:", folder_name).as_str(),
    );
    match serde_json::from_str(&manga_json) {
        Ok(json_value) => match json_value {
            Value::Object(obj) => {
                    ...
                    if let Some(chapter_hash) = data_array.get("hash").and_then(Value::as_str) {
                        if let Some(images1) = data_array.get("data").and_then(Value::as_array) {
                            ...
                            if let Some(images) = data_array.get("data") {
                                let folder_path =
                                    format!("{} - {}Ch.{} - {}", manga_name, vol, chapter, title);
                                ...
                                let start = stdscr().get_max_x() / 3 - images_length as i32 / 2;
...
fn progress_bar_preparation(start: i32, images_length: usize, line: i32) {
    string(
        line,
        0,
        format!("{}|", "-".repeat(start as usize - 1).as_str()).as_str(),
    );
    string(
        line,
...
async fn download_image(
    ...
    let file_name = format!(
        "{} - {}Ch.{} - {} - {}.jpg",
        manga_name, vol, chapter, name, page
    );
    let file_name_brief = format!(
        "{}Ch.{} - {}.jpg",
        vol, chapter, page
    );
    let folder_name = folder_name
        ...
        if current_time.duration_since(last_check_time) >= interval {
            ...
            string(
                ...
                format!(
                    "   {} {}",
                    message,
                    "#".repeat((((stdscr().get_max_x() - message.len() as i32) as f32 / total_size as f32) * downloaded as f32) as usize)
                )
                .as_str(),
            );
        }
    ...
    string(
        ...
        format!(
            "{} {}",
            message,
            "#".repeat((((stdscr().get_max_x() - message.len() as i32) as f32 / total_size as f32) * downloaded as f32) as usize)).as_str()
    );
    string(5 + 1, -1 + start + page as i32, "#");
}
                        ...
                        if let Some(data_array) = obj.get("data").and_then(Value::as_array) {
                            ...
                            if offset_temp >= 500 {
                                if times > 0 {
                                    ...
                                    let data2: Value = serde_json::from_str(&json_2)
                                        .expect("Failed to parse JSON");

                                    let data1_array =
                                        data1.get_mut("data").expect("No 'data' field found");
                                    let data2_array =
                                        data2.get("data").expect("No 'data' field found");

                                    if let Some(data1_array) = data1_array.as_array_mut() {
                                        data1_array.extend(data2_array.as_array().unwrap().clone());
                                    }

                                    json = serde_json::to_string(&data1)
                            ...
                            if times > 0 {
                                let mut data1: Value =
                                    serde_json::from_str(&json).expect("Failed to parse JSON");
                                let data2: Value =
                                    serde_json::from_str(&json_2).expect("Failed to parse JSON");

                                let data1_array =
                                    data1.get_mut("data").expect("No 'data' field found");
                                let data2_array = data2.get("data").expect("No 'data' field found");
